...
func WriteCompactionStats(w io.Writer, cs CompactionStats, elapsed time.Duration) {
	...
	for _, st := range []struct {
		...
		// The logical bytes stand in for the WAL's cost; see CompactionStats
		// for why pebble's WAL.BytesWritten must not be used here.
		{"WAL", cs.WALBytes},
		{"flush -> L0", cs.FlushedBytes},
		{"intra-L0", cs.IntraL0WrittenBytes},
		{"above Lbase", cs.AboveLbaseBytes},
		{"L0 -> Lbase", cs.LbaseWrittenBytes},
		{"Lbase -> L6", cs.CascadeWrittenBytes},
	} {
		if st.bytes == 0 && (st.name == "intra-L0" || st.name == "above Lbase") {
			continue
		}
	...
	fmt.Fprintf(w, "\n  peak L0 sublevels: %d\n", cs.PeakSublevels)
	fmt.Fprintf(w, "  write stalls: %d (%s)", cs.StallCount, cs.StallDuration.Round(time.Millisecond))
	if cs.StallCount > 0 {
		// Which gate: flushing behind the memtables, or compaction behind the
		// flushes. Only the second is the disk's own limit.
		fmt.Fprintf(w, ": memtable queue %d (%s), L0 limit %d (%s)",
			cs.StallMemtableCount, cs.StallMemtableDuration.Round(time.Millisecond),
			cs.StallL0Count, cs.StallL0Duration.Round(time.Millisecond))
	}
	fmt.Fprintln(w)

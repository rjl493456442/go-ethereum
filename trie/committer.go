...
func (c *committer) store(path []byte, n node) (node, *wrapNode) {
	...
	// Gather the trie leaf if required. The fullNode is not checked, as values
	// cannot be stored in a fullNode because all inserted items within the
	// Ethereum protocol have the same key length (32 bytes).
	if c.collectLeaf {

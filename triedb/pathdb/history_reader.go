...
func (r *historyReader) readTrienode(addrHash common.Hash, path string, historyID uint64) ([]byte, error) {
	data, err := rawdb.ReadTrienodeHistory(r.freezer, historyID)
	if err != nil {
		return nil, err
	}
	...
	subset, ok := h.nodes[addrHash]
	if !ok {
		return nil, fmt.Errorf("trienode not found for addrhash: %#x", addrHash)
	}
	return subset[path], nil
}

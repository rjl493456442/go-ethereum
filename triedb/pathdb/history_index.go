...
// indexing.
func newIndexWriter(db ethdb.KeyValueReader, state stateIdent, lastID uint64, bitmapSize int) (*indexWriter, error) {
	blob := readStateIndex(state, db)
	if len(blob) == 0 {
	...
	bw, err := newBlockWriter(indexBlock, lastDesc, lastID, bitmapSize != 0)
	if err != nil {
		return nil, err
	}
	return &indexWriter{
...
func newIndexDeleter(db ethdb.KeyValueReader, state stateIdent, lastID uint64, bitmapSize int) (*indexDeleter, error) {
	blob := readStateIndex(state, db)
	if len(blob) == 0 {

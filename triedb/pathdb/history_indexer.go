...
func loadIndexMetadata(db ethdb.KeyValueReader, typ historyType) *indexMetadata {
	...
	if err := rlp.DecodeBytes(blob, &m); err != nil {
		panic(fmt.Errorf("failed to decode index metadata, %v", err))
	}
	return &m
}
...
func checkVersion(disk ethdb.KeyValueStore, typ historyType) {
	...
	if typ == typeStateHistory {
		...
		rawdb.DeleteStateHistories(disk)
	} else {
		rawdb.DeleteTrienodeHistoryIndexMetadata(disk)
		rawdb.DeleteTrienodeHistories(disk)
	}
	version := "unknown"
	if err == nil {

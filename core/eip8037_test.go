...
func TestParallelReservationOverflowRejected(t *testing.T) {
	...
	if _, err := NewStateProcessor(bc).Process(context.Background(), valid, statedb, nil, vm.Config{}, nil); err != nil {
	...
	_, err = NewStateProcessor(bc).Process(context.Background(), invalid, statedb, nil, vm.Config{}, nil)

...
// MerkleDB is an implementation of Database interface. It leverages both trie and
// state snapshot to provide functionalities for state access in Merkle manner.
...
func NewDatabase(triedb *triedb.Database, codedb *CodeDB) *MerkleDB {
	if codedb == nil {
...
func NewDatabaseForTesting() *MerkleDB {
...
func (db *MerkleDB) WithSnapshot(snapshot *snapshot.Tree) *MerkleDB {
	db.snap = snapshot
	return db
}
...
func (db *MerkleDB) StateReader(stateRoot common.Hash) (StateReader, error) {
	...
	tr, err := newMerkleTreeReader(stateRoot, db.triedb)
	if err != nil {
		return nil, err
	}

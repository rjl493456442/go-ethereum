...
func newWrapTrie(id *trie.ID, db *triedb.Database, prefetch bool) (*wrapTrie, error) {
...
func (tr *wrapTrie) prefetchAccounts(addresses []common.Address) {
	tr.lock.Lock()
	defer tr.lock.Unlock()
...
func (tr *wrapTrie) prefetchStorage(addr common.Address, keys []common.Hash) {
	tr.lock.Lock()
	defer tr.lock.Unlock()
...
func newMerkleHasher(root common.Hash, db *triedb.Database, prefetch bool) (*merkleHasher, error) {
	tr, err := newWrapTrie(trie.StateTrieID(root), db, prefetch)
	if err != nil {
		return nil, err
	}
...
func (h *merkleHasher) openStorageTrie(address common.Address, prefetch bool) (*wrapTrie, error) {
	...
	tr, err := newWrapTrie(id, h.db, h.prefetch && prefetch)
	if err != nil {
		return nil, err
	}
...
func (h *merkleHasher) PrefetchAccount(addresses []common.Address) {
	if !h.prefetch {
		return
	}
	h.acctTrie.prefetchAccounts(addresses)
}
...
func (h *merkleHasher) PrefetchStorage(addr common.Address, keys []common.Hash) {
	if !h.prefetch {
		return
	}

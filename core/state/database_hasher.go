...
//
// Prefetching serves two purposes: preloading the trie paths of mutated
// entries to speed up the hashing/commit phase, and replaying read accesses
// into the trie for witness construction (reads are normally served by the
// flat state and never touch the trie). Whether read accesses should be
// scheduled at all is decided by the caller (e.g. only when a witness is
// being collected).

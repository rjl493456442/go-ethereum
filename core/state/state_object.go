...
func (s *stateObject) GetCommittedState(key common.Hash) common.Hash {
	...
	// is activated. Normal reads are served by the flat state and never
	// touch the trie; without a witness there is no reason to preload the
	// paths of read accesses.
	if s.db.witness != nil {
		if prefetch, ok := s.db.hasher.(Prefetcher); ok {
			prefetch.PrefetchStorage(s.address, []common.Hash{key})
		}
	}
	return value
}
...
func (s *stateObject) finalise() {
	...
	// Schedule the modified storage slots for prefetching if it's enabled.
	if prefetch, ok := s.db.hasher.(Prefetcher); ok && len(slotsToPrefetch) > 0 {
		prefetch.PrefetchStorage(s.address, slotsToPrefetch)
	}
}

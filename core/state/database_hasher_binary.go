...
type warpBinTrie struct {
	*bintrie.BinaryTrie
	lock       sync.Mutex // Lock protecting the prefetcher handoff below
	prefetcher *prefetcher
}

// newWrapBinTrie creates a binary trie with the optional prefetcher enabled.
func newWrapBinTrie(root common.Hash, db *triedb.Database, prefetch bool) (*warpBinTrie, error) {
	t, err := bintrie.NewBinaryTrie(root, db, db.BinTrieGroupDepth())
	if err != nil {
		return nil, err
	}
	var p *prefetcher
	if prefetch && root != types.EmptyBinaryHash {
		p = newPrefetcher(t)
	}
...
//
// term is safe for concurrent invocation, e.g. the deferred prefetcher
// termination in the block processing racing with the state copy made
// by the transaction pre-executor.
...
func (tr *warpBinTrie) prefetchAccounts(addresses []common.Address) {
	tr.lock.Lock()
	defer tr.lock.Unlock()
	...
	tr.prefetcher.scheduleAccounts(addresses)
}

func (tr *warpBinTrie) prefetchStorage(addr common.Address, keys []common.Hash) {
	tr.lock.Lock()
	defer tr.lock.Unlock()
	...
	tr.prefetcher.scheduleSlots(addr, keys)
}
...
type binaryHasher struct {
	...
	// lock serializes mutations on the unified trie. Unlike the two-layer
	// merkle hasher where concurrent storage updates land in per-account
	// tries, all writes here target a single trie and the storage updates
	// may be flushed concurrently (e.g. by StateDB.IntermediateRoot).
	lock sync.Mutex
}

func newBinaryHasher(root common.Hash, db *triedb.Database, prefetch bool) (*binaryHasher, error) {
	tr, err := newWrapBinTrie(root, db, prefetch)
	if err != nil {
		return nil, err
	}
	return &binaryHasher{
...
func (h *binaryHasher) PrefetchAccount(addresses []common.Address) {
	if !h.prefetch {
		return
	}
	h.trie.prefetchAccounts(addresses)
}
...
func (h *binaryHasher) PrefetchStorage(addr common.Address, keys []common.Hash) {
	if !h.prefetch {
		return
	}
	h.trie.prefetchStorage(addr, keys)
}

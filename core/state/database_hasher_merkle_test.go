...
// hasherTestConfig captures the prefetch flag varied across subtests.
type hasherTestConfig struct {
	name     string
	prefetch bool
}

// hasherTestConfigs enumerates the interesting prefetch combinations:
//   - no prefetch at all
//   - prefetch enabled (both mutated and read paths may be scheduled)
...
func newTestHasher(t *testing.T, db *triedb.Database, root common.Hash, cfg hasherTestConfig) *merkleHasher {
	...
	h, err := newMerkleHasher(root, db, cfg.prefetch)
	if err != nil {
		t.Fatal(err)
	}
...
func commitAndReopen(t *testing.T, h *merkleHasher, cfg hasherTestConfig) *merkleHasher {
	...
	h2, err := newMerkleHasher(root, h.db, cfg.prefetch)
	if err != nil {
		t.Fatal(err)
	}
...
func makeBaseState(t *testing.T, cfg hasherTestConfig) *merkleHasher {
	...
	noPrefetch := hasherTestConfig{"base", false}
...
func TestMerkleHasherBasic(t *testing.T) {
	for _, cfg := range hasherTestConfigs {
		t.Run(cfg.name, func(t *testing.T) {
			...
			if cfg.prefetch {
				h.PrefetchStorage(hasherAddr1, []common.Hash{hasherSlot3})
				h.PrefetchAccount([]common.Address{hasherAddr1, hasherAddr3})
			}
...
// state and does not leak goroutines. Whether read accesses are scheduled at
// all is decided by the caller (e.g. only when a witness is collected).
func TestMerkleHasherPrefetchReadOnly(t *testing.T) {
	cfg := hasherTestConfig{"readProcessed", true}
	h := makeBaseState(t, cfg)
	rootBefore := h.Hash()

	// Prefetch addr1's account and storage (read-only, never mutated below).
	h.PrefetchAccount([]common.Address{hasherAddr1, hasherAddr2})
	h.PrefetchStorage(hasherAddr1, []common.Hash{hasherSlot1, hasherSlot2})
	...
	h2 := commitAndReopen(t, h, hasherTestConfig{"verify", false})
	if h2.Hash() != root {
...
func TestMerkleHasherDeleteAccount(t *testing.T) {
	for _, cfg := range hasherTestConfigs {
		t.Run(cfg.name, func(t *testing.T) {
			...
			if cfg.prefetch {
				h.PrefetchAccount([]common.Address{hasherAddr1})
				h.PrefetchStorage(hasherAddr1, []common.Hash{hasherSlot1, hasherSlot2})
			}
...
func TestMerkleHasherPrefetchDeterminism(t *testing.T) {
	...
	for _, cfg := range hasherTestConfigs {
		...
		if cfg.prefetch {
			h.PrefetchAccount([]common.Address{hasherAddr1, hasherAddr3})
			h.PrefetchStorage(hasherAddr1, []common.Hash{hasherSlot3})
			h.PrefetchStorage(hasherAddr3, []common.Hash{hasherSlot1})
		}
...
func TestMerkleHasherCopy(t *testing.T) {
	cfg := hasherTestConfig{"prefetch", true}
	h := makeBaseState(t, cfg)
...
func TestMerkleHasherWitness(t *testing.T) {
	h := makeBaseState(t, hasherTestConfig{"prefetch", true})
	...
	h.PrefetchStorage(hasherAddr1, []common.Hash{hasherSlot1})
	...
	prover, err := newMerkleHasher(h.root, h.db, false)
	if err != nil {
		t.Fatal(err)
	}

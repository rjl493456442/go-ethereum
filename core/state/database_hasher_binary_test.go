...
func newTestBinaryHasher(t *testing.T, db *triedb.Database, root common.Hash, cfg hasherTestConfig) *binaryHasher {
	...
	h, err := newBinaryHasher(root, db, cfg.prefetch)
	if err != nil {
		t.Fatal(err)
	}
...
func commitAndReopenBinary(t *testing.T, h *binaryHasher, cfg hasherTestConfig) *binaryHasher {
	...
	h2, err := newBinaryHasher(root, h.db, cfg.prefetch)
	if err != nil {
		t.Fatal(err)
	}
...
// state. Whether read accesses are scheduled at all is decided by the caller
// (e.g. only when a witness is collected).
func TestBinaryHasherPrefetchReadOnly(t *testing.T) {
	...
	h2 := commitAndReopenBinary(t, h, hasherTestConfig{"verify", false})
	if h2.Hash() != root {
...
func TestBinaryHasherWitness(t *testing.T) {
	// collectWitness builds a witness. The prefetchRead flag simulates the
	// caller-side gating: read accesses are replayed into the hasher only
	// when a witness is being collected.
	collectWitness := func(prefetchRead bool) int {
		cfg := hasherTestConfig{"witness", true}
		h := makeBinaryBaseState(t, cfg)

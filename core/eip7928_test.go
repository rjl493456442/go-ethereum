...
func assertParallelEquiv(t *testing.T, gspec *Genesis, engine consensus.Engine, block *types.Block) {
	...
	parRes, err := NewStateProcessor(bc).Process(context.Background(), block, parState, nil, vm.Config{}, nil)
	if err != nil {
	...
	seqRes, err := NewStateProcessor(bc).Process(context.Background(), block, seqState, nil, vm.Config{DisableParallelExecution: true}, nil)
	if err != nil {

...
// store before it's flushed to disk. Flushing fsyncs every freezer table, so it
// is amortized across insertions instead of being run on each of them. Data
// written past the last flush is discarded by freezer recovery after an unclean
// shutdown and downloaded again.
const ancientSyncThreshold = 2 * 1024 * 1024 * 1024

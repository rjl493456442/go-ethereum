...
var (
	...
	// errPartialReadNowAllowed is returned if partial read is performed on the
	// compressed freezer table.
	errPartialReadNowAllowed = errors.New("partial read now allowed")
)
...
// and value offsets.
func (t *freezerTable) RetrieveBytes(id uint64, offset uint64, length uint64) ([]byte, error) {
	t.lock.RLock()
	defer t.lock.RUnlock()
	...
	if !t.config.noSnappy {
		return nil, errPartialReadNowAllowed
	}
	items, hidden := t.items.Load(), t.itemHidden.Load()
	if items <= id || hidden > id {
		return nil, errOutOfBounds
	}

	// Retrieves the index entries for the specified ID and its immediate successor.
	// The pair of indices can uniquely locate the element position in the data file.
	indices, err := t.getIndices(id, 1)
	if err != nil {
		return nil, err
	}
	...
	// Apply the partial read segment limits

...
// ConstructionAccountAccesses contains post-block account state for mutations as
// well as all storage keys that were read during execution. It is used when
// building block access list during execution.
type ConstructionAccountAccesses struct {
	...
	// CodeChanges contains the post-state code changes of an account keyed
	// by tx index.
...
func NewConstructionBlockAccessList() *ConstructionBlockAccessList {
	return &ConstructionBlockAccessList{make(map[common.Address]*ConstructionAccountAccesses), 0}
}

...
// PrecompileCache represents the cache of precompile execution results.
type PrecompileCache interface {
	// Load retrieves the cached precompile execution result for the given
	// inputs. Returns the result and true if found, or nil and false if
	// not cached.
	//
	// Note the returned object is deep-copied and safe to modify.
	Load(name string, input []byte) ([]byte, bool)

	// Store saves the precompile execution result for the given input.
	Store(name string, input []byte, result []byte)
}
...
type PrecompiledContract interface {
	...
	Run(input []byte, cache PrecompileCache) ([]byte, error) // Run runs the precompiled contract
	Name() string
}
...
func (c *ecrecover) Run(input []byte, cache PrecompileCache) ([]byte, error) {
	cached, found := cache.Load(c.Name(), input)
	if found {
		return cached, nil
	}
	...
	cache.Store(c.Name(), input, result)
...
func (c *sha256hash) Run(input []byte, cache PrecompileCache) ([]byte, error) {
	cached, found := cache.Load(c.Name(), input)
	if found {
		return cached, nil
	}
...
func (c *ripemd160hash) Run(input []byte, cache PrecompileCache) ([]byte, error) {
	cached, found := cache.Load(c.Name(), input)
	if found {
		return cached, nil
	}
...
func (c *dataCopy) Run(in []byte, _ PrecompileCache) ([]byte, error) {
...
func (c *bigModExp) Run(input []byte, cache PrecompileCache) ([]byte, error) {
	cached, found := cache.Load(c.Name(), input)
	if found {
		return cached, nil
	}
...
func (c *bn256AddIstanbul) Run(input []byte, cache PrecompileCache) ([]byte, error) {
	cached, found := cache.Load(c.Name(), input)
	if found {
		return cached, nil
	}
...
func (c *bn256AddByzantium) Run(input []byte, cache PrecompileCache) ([]byte, error) {
	cached, found := cache.Load(c.Name(), input)
	if found {
		return cached, nil
	}
...
func (c *bn256ScalarMulIstanbul) Run(input []byte, cache PrecompileCache) ([]byte, error) {
	cached, found := cache.Load(c.Name(), input)
	if found {
		return cached, nil
	}
...
func (c *bn256ScalarMulByzantium) Run(input []byte, cache PrecompileCache) ([]byte, error) {
	cached, found := cache.Load(c.Name(), input)
	if found {
		return cached, nil
	}
...
func (c *bn256PairingIstanbul) Run(input []byte, cache PrecompileCache) ([]byte, error) {
	cached, found := cache.Load(c.Name(), input)
	if found {
		return cached, nil
	}
...
func (c *bn256PairingByzantium) Run(input []byte, cache PrecompileCache) ([]byte, error) {
	cached, found := cache.Load(c.Name(), input)
	if found {
		return cached, nil
	}
...
func (c *blake2F) Run(input []byte, cache PrecompileCache) ([]byte, error) {
	cached, found := cache.Load(c.Name(), input)
	if found {
		return cached, nil
	}
...
func (c *bls12381G1Add) Run(input []byte, cache PrecompileCache) ([]byte, error) {
	cached, found := cache.Load(c.Name(), input)
	if found {
		return cached, nil
	}
...
func (c *bls12381G1MultiExp) Run(input []byte, cache PrecompileCache) ([]byte, error) {
	cached, found := cache.Load(c.Name(), input)
	if found {
		return cached, nil
	}
...
func (c *bls12381G2Add) Run(input []byte, cache PrecompileCache) ([]byte, error) {
	cached, found := cache.Load(c.Name(), input)
	if found {
		return cached, nil
	}
...
func (c *bls12381G2MultiExp) Run(input []byte, cache PrecompileCache) ([]byte, error) {
	cached, found := cache.Load(c.Name(), input)
	if found {
		return cached, nil
	}
...
func (c *bls12381Pairing) Run(input []byte, cache PrecompileCache) ([]byte, error) {
	cached, found := cache.Load(c.Name(), input)
	if found {
		return cached, nil
	}
...
func (c *bls12381MapG1) Run(input []byte, cache PrecompileCache) ([]byte, error) {
	cached, found := cache.Load(c.Name(), input)
	if found {
		return cached, nil
	}
...
func (c *bls12381MapG2) Run(input []byte, cache PrecompileCache) ([]byte, error) {
	cached, found := cache.Load(c.Name(), input)
	if found {
		return cached, nil
	}
...
func (b *kzgPointEvaluation) Run(input []byte, cache PrecompileCache) ([]byte, error) {
	cached, found := cache.Load(b.Name(), input)
	if found {
		return cached, nil
	}
	result, err := b.run(input)
	if err != nil {
		return nil, err
	}
	cache.Store(b.Name(), input, result)
...
func (c *p256Verify) Run(input []byte, cache PrecompileCache) ([]byte, error) {
	cached, found := cache.Load(c.Name(), input)
	if found {
		return cached, nil
	}

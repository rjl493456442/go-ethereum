...
func (g *GasBudget) ChargeRegular(r uint64) bool {
	_, ok := g.Charge(GasCosts{RegularGas: r})
	return ok
}
...
func (g *GasBudget) ChargeState(s uint64) bool {
	_, ok := g.Charge(GasCosts{StateGas: s})
	return ok
}

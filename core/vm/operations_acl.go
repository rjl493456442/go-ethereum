...
func gasCallEIP7702(evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	...
	// Perform stateless gas measurement, it should be done before the EIP-7702
	// target resolution
	statelessCost, err := gasCallStateless(evm, stack, mem, memorySize)
	if err != nil {
		return 0, err
	}

	// Perform EIP-7702 checks, checking if code is a delegation and
	// if so, charge for resolution.
	...
	// Perform stateless gas measurement, it should be done before the EIP-7702
	// target resolution
	statefulCost, err := gasCallStateful(evm, stack)
	if err != nil {
		return 0, err
	}
	callCost, overflow := math.SafeAdd(statelessCost, statefulCost)
	if overflow {
		return 0, ErrGasUintOverflow
	}

	// Perform the 63/64 rule, deriving the amount of gas for nested call
	evm.callGasTemp, err = callGas(evm.chainRules.IsEIP150, contract.Gas, callCost, stack.Back(0))
	if err != nil {
		return 0, err
	}
	...
	if totalCost, overflow = math.SafeAdd(totalCost, callCost); overflow {
		return 0, ErrGasUintOverflow
	}

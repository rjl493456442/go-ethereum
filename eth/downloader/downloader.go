...
func (d *Downloader) syncToHead() (err error) {
	...
	// Disable pruning mode if the configured chain cutoff is even higher
	// than the target of synchronization.
	if mode == ethconfig.SnapSync && d.chainCutoffNumber > height {
	...
	for {
		...
		case task := <-d.headerProcCh:
			...
			for len(headers) > 0 {
				...
				// TODO verify the cutoff block hash in the skeleton sync
